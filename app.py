...
# 2. Pengunduhan File SEGY dari Hugging Face
DATA_URL = "https://huggingface.co/datasets/Rafisuper/SORA/resolve/main/Citra%20Sumur.segy"
...
def download_data():
  if not os.path.exists(DATA_PATH):
    with st.spinner("Mengunduh data seismik dari Hugging Face..."):
...
class RealVolveEngine:
  ...
  def __init__(
      self,
      excel_path="Volve production data.xlsx",
      segy_path="Citra Sumur.segy",
  ):
  ...
  def load_and_clean_data(self):
    if not os.path.exists(self.excel_path):
      st.error(
          f"File '{self.excel_path}' tidak ditemukan! Pastikan file berada di"
          " repositori."
      )
      st.stop()
    ...
    try:
      ...
      df_clean["WATER_CUT"] = df_clean["BORE_WAT_VOL"] / (
          df_clean["BORE_OIL_VOL"] + df_clean["BORE_WAT_VOL"] + 1e-6
      )

      st.sidebar.success(
          f"Data berhasil dimuat! Membaca {len(df_clean):,} Sampel"
      )
      return df_clean
  ...
  def load_seismic_4d_amplitude(self):
    ...
    try:
      with segyio.open(
          self.segy_path, mode="r", ignore_geometry=True
      ) as f:
        n_traces = len(f.trace)
        mid_trace = f.trace[n_traces // 2]
        delta_amplitude_mean = float(np.mean(mid_trace))

        st.sidebar.success("Data Seismik 4D Berhasil Terbaca!")
...
class SEIRAAICore:
  ...
  def train(self, data, n_iter=15, cv_folds=3):
    # Catatan: n_iter dan cv_folds dikurangi sedikit agar proses build di Streamlit Cloud tidak timeout
    if data.empty:
    ...
    self.feature_importances_df = pd.DataFrame(
        {"Fitur": self.feature_cols, "Importance": importances}
  ...
  def optimize_eor_strategy(self, current_state):
    ...
    if viscosity > 20.0:
      return (
          "Thermal EOR",
          f"Viskositas minyak tinggi ({viscosity} cP). Perlu injeksi uap panas"
          " untuk menurunkan viskositas fluida.",
      )
    elif wc > 0.60:
      return (
          "Polymer",
          f"Water Cut sangat tinggi ({wc*100:.1f}%). Diperlukan polymer gel untuk"
          f" menyumbat zona berpermeabilitas tinggi ({permeability} mD) agar"
          " menyapu sisa minyak.",
      )
    elif press > 220 and viscosity < 10.0:
      return (
          "CO2-Miscible Injection",
          f"Tekanan reservoir mencukupi ({press:.1f} bar > MMP) dan viskositas"
          f" minyak rendah ({viscosity} cP) pada formasi {rock_type}. Respon"
          f" Seismik 4D (ΔAmp = {delta_amp_4d:.4f}) mengonfirmasi penyebaran"
          " fluida optimal.",
      )
    elif permeability > 50 and wc <= 0.60:
      return (
          "ASP (Alkaline-Surfactant-Polymer) Flooding",
          f"Kondisi batuan {rock_type} (k = {permeability} mD) mendukung"
          " injeksi kimia untuk menurunkan tegangan antarmuka (IFT) dan"
          " meningkatkan efisiensi penyapuan.",
      )
    else:
      return (
          "Waterflooding / Immiscible Gas",
          "Tekanan reservoir rendah dan permeabilitas terbatas. Lakukan"
          " pemeliharaan tekanan dasar terlebih dahulu.",
      )
...
class AutomationEngine:
  ...
  def check_well_health(pressure, choke_size):
    if choke_size > 80.0 and pressure < 150.0:
      return (
          "CRITICAL",
          "Choke terbuka lebar (>80%) tetapi tekanan dasar sumur drop (<150"
          " bar). Bahaya liquid loading!",
      )
    elif choke_size > 90.0:
      return (
          "WARNING",
          "Bukaan choke mendekati kapasitas maksimal. Lakukan evaluasi"
          " kerugian gesekan tubing.",
      )
    else:
      return (
          "NORMAL",
          "Operasi tekanan dan aliran sumur berada pada rentang optimal.",
      )
  ...
  def predict_pump_failure(vibration_rms, bearing_temp, inlet_pressure):
    ...
    if vibration_rms > 7.1:
      faults.append(
          "Vibrasi Ekstrem (Misalignment / Severe Bearing Failure)"
      )
    ...
    if vibration_rms > 7.1 or bearing_temp > 95.0 or inlet_pressure < 2.0:
      ...
      action = (
          "Emergency Shutdown! Switch ke pompa cadangan dan lakukan"
          " overhaul/inspeksi mechanical seal."
      )
    elif vibration_rms >= 4.5 or bearing_temp >= 80.0:
      status = "WARNING"
      rul = "120 - 168 Jam (1 Minggu)"
      action = (
          "Jadwalkan pemeliharaan preventif, lakukan re-greasing, dan"
          " re-alignment rotor."
      )
    else:
      status = "NORMAL"
    ...
    return {
        "status": status,
        "detected_faults": (
            faults if faults else ["Tidak ada indikasi kerusakan"]
        ),
        "rul_estimate": rul,
  ...
  def calculate_carbon_footprint(self, gas_vol_sm3_day, days=30):
    ...
    return {
        ...
        "esg_compliance_status": (
            "COMPLIANT (Net Positive Storage)"
            if net_carbon_sequestered > emitted_carbon
            else "NON-COMPLIANT"
        ),
    }
...
st.caption(
    "Sistem integrasi Artificial Intelligence dalam operasi Enhanced Oil"
    " Recovery (EOR)"
)
...
dataset, ai_core, r2_score_val, rmse_val, delta_amp_4d_val = (
    initialize_system()
)
...
pressure = st.sidebar.slider(
    "Tekanan Dasar Sumur / BHP (bar)", 50.0, 350.0, 240.0
)
...
water_rate = st.sidebar.slider(
    "Laju Air / Water Rate (Sm³/day)", 0.0, 5000.0, 800.0
)
...
rock_type = st.sidebar.selectbox(
    "Tipe Batuan Reservoir", ["Sandstone", "Carbonate"]
)
...
pump_bearing_temp = st.sidebar.slider(
    "Temperatur Bearing (°C)", 30.0, 120.0, 65.0
)
pump_vibration_rms = st.sidebar.slider(
    "Vibrasi Pompa (mm/s RMS)", 0.5, 12.0, 2.3
)
...
well_status_code, well_status_msg = AutomationEngine.check_well_health(
    pressure, choke_size
)
pump_diag = AutomationEngine.predict_pump_failure(
    pump_vibration_rms, pump_bearing_temp, pump_inlet_press
)
...
with kpi4:
  st.metric(
      "Status ESG Compliance",
      "COMPLIANT"
      if "COMPLIANT" in esg_res["esg_compliance_status"]
      else "NON-COMPLIANT",
  )
...
with acc2:
  st.metric("Cross-Validation R² (mean)", f"{ai_core.cv_r2_mean:.4f}")
with acc3:
  st.metric("Cross-Validation R² (std dev)", f"± {ai_core.cv_r2_std:.4f}")

with st.expander("Lihat Hyperparameter Terbaik"):
...
with col_left:
  ...
  st.write(
      f"• **Indikasi Kerusakan:** {', '.join(pump_diag['detected_faults'])}"
  )
...
with col_right:
  ...
  st.write(
      f"• **Gross CO2/Gas Handled:** {esg_res['gross_co2_injected_tons']:,} Ton"
  )
  st.write(
      "• **Net Carbon Sequestered:**"
      f" {esg_res['net_carbon_sequestered_tons']:,} Ton"
  )
